...
* **3**    download `Flash algorithm`_ ,enter the folder scripts. use command "python generate_blobs.py + FLM(generate in last step) fileto generate 
py_blob_orig.py.
* **4**    create new python file in the  /pyocd/target/builtub in pyOCD project. the file format can be referenced to the other target file . then you should create class for i.mxrt1060 MCU you can copy from other target file with similar .then rename it(for example target_MIMXRT1052xxxxB.py)
